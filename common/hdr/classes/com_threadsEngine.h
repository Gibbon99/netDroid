...
public:
	...
	bool lockMutex (const std::string &mutexName);

	bool lockMutex (SDL_mutex *mutexName);

    bool unLockMutex(SDL_mutex *mutexName);

	bool unLockMutex (const std::string &mutexName);
