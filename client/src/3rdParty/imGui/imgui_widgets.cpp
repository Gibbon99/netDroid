...
// See the series of events below and the corresponding textureState reported by dear imgui:
//------------------------------------------------------------------------------------------------------------------------------------------------
...
bool ImGui::ButtonBehavior(const ImRect& bb, ImGuiID id, bool* out_hovered, bool* out_held, ImGuiButtonFlags flags)
{
    ...
    if (g.NavActivateDownId == id)
    {
        ...
        {
            // Set editorActive id so it can be queried by user via IsItemActive(), equivalent of holding the mouse button.
            pressed = true;
            SetActiveID(id, window);
...
bool ImGui::ImageButton(ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, int frame_padding, const ImVec4& bg_col, const ImVec4& tint_col)
{
    ...
    // Default to using textureID ID as ID. User can still push string/integer prefixes.
...
{
    // FIXME-OPT: we could pre-compute the indices to fasten this. But only 1 editorActive combo means the waste is limited.
...
// This is called by DragBehavior() when the widget is editorActive (held by mouse or being manipulated with Nav controls)
...
// Create text input in place of another editorActive widget (e.g. used when doing a CTRL+Click on drag/slider widgets)
...
bool ImGui::TempInputText(const ImRect& bb, ImGuiID id, const char* label, char* buf, int buf_size, ImGuiInputTextFlags flags)
{
    ...
    if (init)
    {
        // First frame we started displaying the InputText widget, we expect it to take the editorActive id.
...
// - When editorActive, hold on a privately held copy of the text (and apply back to 'buf'). So changing 'buf' while the InputText is editorActive has no effect.
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    // We are only allowed to access the textureState if we are already the editorActive widget.
    ...
    {
        // Access textureState even if we don't own it yet.
    ...
    // FIXME-OPT: Because our selection/cursor code currently needs the wide text we need to convert it when editorActive, which is not ideal :(
    ...
    if (g.ActiveId == id)
    {
        ...
        // Although we are editorActive we don't prevent mouse from hovering other elements unless we are interacting right now with the widget.
        ...
        else if (hovered && io.MouseClickedCount[0] >= 2 && !io.KeyShift)
        {
            ...
            if ((multiclick_count % 2) == 0)
            {
                ...
                //textureState->OnKeyPressed(STB_TEXTEDIT_K_WORDRIGHT | STB_TEXTEDIT_K_SHIFT);
    ...
    if (g.ActiveId == id)
    {
        ...
        if (apply_edit_back_to_user_buffer)
        {
            ...
            // Note that as soon as the input box is editorActive, the in-widget value gets priority over any underlying modification of the input buffer
            // FIXME: We actually always render 'buf' when calling DrawList->AddText, making the comment above incorrect.
            // FIXME-OPT: CPU waste to do this every time the widget is editorActive, should mark dirty textureState from the stb_textedit callbacks.
            if (!is_readonly)
            {
    ...
    // Release editorActive ID at the end of the function (so e.g. pressing Return still does a final application of the value)
    ...
    // Render text. We currently only render selection when the widget is editorActive or while scrolling.
    ...
    if (is_multiline)
    {
        ...
        // ...and then we need to undo the group overriding last item data, which gets a bit messy as EndGroup() tries to forward scrollbar being editorActive...
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    // When picker is being actively used, use its editorActive id so IsItemActive() will function on ColorEdit4().
...
bool ImGui::TreeNodeBehaviorIsOpen(ImGuiID id, ImGuiTreeNodeFlags flags)
{
    ...
    if (g.NextItemData.Flags & ImGuiNextItemDataFlags_HasOpen)
    {
        ...
        {
            // We treat ImGuiCond_Once and ImGuiCond_FirstUseEver the same because tree node textureState are not saved persistently.
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
    ...
    // We set ImGuiButtonFlags_PressedOnClickRelease on OpenOnDoubleClick because we want the item to be editorActive on the initial MouseDown in order for drag and drop to work.
...
// Set next TreeNode/CollapsingHeader open textureState.
...
// Do not mistake this with the Open textureState of the header itself, which you can adjust with SetNextItemOpen() or ImGuiTreeNodeFlags_DefaultOpen.
...
static void ImGui::TabBarLayout(ImGuiTabBar* tab_bar)
{
    ...
    for (int tab_n = 0; tab_n < tab_bar->Tabs.Size; tab_n++)
    {
        ...
        // and we cannot wait for the next BeginTabItem() call. We cannot compute this width within TabBarAddTab() because font size depends on the editorActive window.
    ...
    // If we have lost the selected tab, select the next most recently editorActive one
...
bool    ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags)
{
    ...
    // We don't have CPU clipping primitives to clip the CloseButton (until it becomes a textureID), so need to add an extra draw call (temporary in the case of vertical animation)
    ...
    // (We test IsItemHovered() to discard e.g. when another item is editorActive or drag and drop over the tab bar, which g.HoveredId ignores)
...
void ImGui::TabItemLabelAndCloseButton(ImDrawList* draw_list, const ImRect& bb, ImGuiTabItemFlags flags, ImVec2 frame_padding, const char* label, ImGuiID tab_id, ImGuiID close_button_id, bool is_contents_visible, bool* out_just_closed, bool* out_text_clipped)
{
    ...
    // In Style V2 we'll have full override of all colors per textureState (e.g. focused, selected)
    ...
    // Return clipped textureState ignoring the close button
    if (out_text_clipped)
    {
