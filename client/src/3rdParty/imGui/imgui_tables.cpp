...
void ImGui::TableUpdateLayout(ImGuiTable* table)
{
    ...
    for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
    {
        ...
        // Update Enabled textureState, mark settings and sort specs dirty
...
void ImGui::TableSetupColumn(const char* label, ImGuiTableColumnFlags flags, float init_width_or_weight, ImGuiID user_id)
{
    ...
    if (table->IsInitializing)
    {
        ...
        // Init default visibility/sort textureState
...
// Change user accessible enabled/disabled textureState of a column (often perceived as "showing/hiding" from users point of view)
// Note that end-user can use the context menu to change this themselves (right-click in headers, or right-click in columns body with ImGuiTableFlags_ContextMenuInBody)
// - Require table to have the ImGuiTableFlags_Hideable flag because we are manipulating user accessible textureState.
...
// We allow querying for an extra column in order to poll the IsHovered textureState of the right-most section
...
// FIXME-TABLE FIXME-OPT: Could probably shortcut some things for non-editorActive or clipped columns.
...
void ImGui::TableSetColumnWidth(int column_n, float width)
{
    ...
    // - F1 F2 F3  resize from F3|          --> ok: alter ->WidthRequested of Fixed column. If editorActive, ScrollX extent can be altered.
    // - F1 F2 W3  resize from F1| or F2|   --> ok: alter ->WidthRequested of Fixed column. If editorActive, ScrollX extent can be altered, but it doesn't make much sense as the Stretch column will always be minimal size.
...
void ImGui::TableDrawContextMenu(ImGuiTable* table)
{
    ...
    if (table->Flags & ImGuiTableFlags_Hideable)
    {
        ...
        for (int other_column_n = 0; other_column_n < table->ColumnsCount; other_column_n++)
        {
            ...
            // Make sure we can't hide the last editorActive column
...
// Restore initial textureState of table (with or without saved settings)
...
void ImGui::TableSaveSettings(ImGuiTable* table)
{
    ...
    for (int n = 0; n < table->ColumnsCount; n++, column++, column_settings++)
    {
        ...
        // We skip saving some data in the .ini file when they are unnecessary to restore our textureState.
...
void ImGui::BeginColumns(const char* str_id, int columns_count, ImGuiOldColumnFlags flags)
{
    ...
    // Set textureState for first column
