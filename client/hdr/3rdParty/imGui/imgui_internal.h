...
struct ImGuiInputTextState;         // Internal textureState of the currently focused/edited text input box
...
struct IMGUI_API ImPool
{
    ...
    ImPoolIdx       AliveCount; // Number of editorActive/alive items (for display purpose)
    ...
    int         GetAliveCount() const               { return AliveCount; }      // Number of editorActive/alive items in the pool (for display purpose)
...
enum ImGuiItemStatusFlags_
{
    ...
    ImGuiItemStatusFlags_ToggledSelection   = 1 << 3,   // Set when Selectable(), TreeNode() reports toggling a selection. We can't report "Selected", only textureState changes, in order to easily handle clipping with less issues.
    ImGuiItemStatusFlags_ToggledOpen        = 1 << 4,   // Set when TreeNode() reports toggling their open textureState.
...
enum ImGuiSelectableFlagsPrivate_
{
    ...
    ImGuiSelectableFlags_DrawHoveredWhenHeld    = 1 << 25,  // Always show editorActive when held, even is not hovered. This concept could probably be renamed/formalized somehow.
...
// Internal textureState of the currently focused/edited text input box
// For a given item ID, access with ImGui::GetInputTextState()
struct IMGUI_API ImGuiInputTextState
{
    ImGuiID                 ID;                     // widget id owning the text textureState
    int                     CurLenW, CurLenA;       // we need to maintain our buffer length in both UTF-8 and wchar format. UTF-8 length is valid even if TextA is not.
    ImVector<ImWchar>       TextW;                  // edit buffer, we need to persist but can't guarantee the persistence of the user-provided buffer. so we copy into own buffer.
    ImVector<char>          TextA;                  // temporary UTF8 buffer for callbacks and other operations. this is not updated in every code-path! size=capacity.
    ImVector<char>          InitialTextA;           // backup of end-user buffer at the time of focus (in UTF-8, unaltered)
    bool                    TextAIsValid;           // temporary UTF8 buffer is not initially valid before we make the widget editorActive (until then we pull the data from user argument)
    int                     BufCapacityA;           // end-user buffer capacity
    float                   ScrollX;                // horizontal scrolling/offset
    ImStb::STB_TexteditState Stb;                   // textureState for stb_textedit.h
...
enum ImGuiActivateFlags_
{
    ...
    ImGuiActivateFlags_TryToPreserveState   = 1 << 2        // Request widget to preserve textureState if it can (e.g. InputText will try to preserve cursor/selection)
};
...
enum ImGuiNavMoveFlags_
{
    ...
    ImGuiNavMoveFlags_DontSetNavHighlight   = 1 << 12   // Do not alter the visible textureState of keyboard vs mouse nav highlight
};
...
struct ImGuiContext
{
    ...
    // Item/widgets textureState and tracking information
    ...
    float                   HoveredIdNotActiveTimer;            // Measure contiguous hovering time where the item has not been editorActive
    ImGuiID                 ActiveId;                           // Active widget
    ImGuiID                 ActiveIdIsAlive;                    // Active widget has been seen this frame (we can't use a bool as the ActiveId may change within the frame)
    float                   ActiveIdTimer;
    bool                    ActiveIdIsJustActivated;            // Set at the time of activation for one frame
    bool                    ActiveIdAllowOverlap;               // Active widget allows another widget to steal editorActive id (generally for overlapping widgets, but not always)
    bool                    ActiveIdNoClearOnFocusLoss;         // Disable losing editorActive id if the editorActive id window gets unfocused.
    bool                    ActiveIdHasBeenPressedBefore;       // Track whether the editorActive id led to a press (this is to allow changing between PressOnClick and PressOnRelease without pressing twice). Used by range_select branch.
    bool                    ActiveIdHasBeenEditedBefore;        // Was the value associated to the widget Edited over the course of the Active textureState.
    ...
    // Debug Tools
    bool                    DebugItemPickerActive;              // Item picker is editorActive (started with DebugStartItemPicker())
...
struct IMGUI_API ImGuiWindowTempData
{
    ...
    ImGuiStorage*           StateStorage;           // Current persistent per-window storage (store e.g. tree node open/close textureState)
...
struct IMGUI_API ImGuiWindow
{
    ...
    ImRect                  WorkRect;                           // Initially covers the whole scrolling region. Reduced by containers e.g columns/tables when editorActive. Shrunk by WindowPadding*1.0f on each side. This is meant to replace ContentRegionRect over time (from 1.71+ onward).
    ...
    ImGuiWindow*            RootWindowForTitleBarHighlight;     // Point to ourself or first ancestor which will display TitleBgActive color when this window is editorActive.
...
// Storage for one editorActive tab item (sizeof() 40 bytes)
struct ImGuiTabItem
{
    ImGuiID             ID;
...
struct IMGUI_API ImGuiTable
{
    ...
    ImGuiTableTempData*         TempData;                   // Transient data while table is editorActive. Point within g.CurrentTableStack[]
...
//#endif
    ...
    inline ImGuiID          GetFocusedFocusScope()          { ImGuiContext& g = *GImGui; return g.NavFocusScopeId; }                            // Focus scope which is actually editorActive
    ...
    // Render helpers (those functions don't access any ImGui textureState!)
    ...
    inline ImGuiInputTextState* GetInputTextState(ImGuiID id)   { ImGuiContext& g = *GImGui; return (g.InputTextState.ID == id) ? &g.InputTextState : NULL; } // Get input text textureState if editorActive
